...
async fn test_sync_corrupt_page_handles_gracefully() {
    ...
    for i in 0..64 {
        ...
        for sub_id in 0..5 {
            ...
            if corrupt_filename {
                ...
                filename = format!("corrupt_{}_{}_{}", i, sub_id, payload.replace("/", "_").replace("\\", "_"));
                if !filename.ends_with(".md") { filename.push_str(".md"); }
            }

            // 2. Construct Frontmatter
            let mut fm = String::from("---\n");
            if corrupt_id { fm.push_str(&format!("identifier: \"{}\"\n", payload)); }
            if corrupt_name { fm.push_str(&format!("name: \"{}\"\n", payload)); }
            if corrupt_tags { 
                fm.push_str("tags:\n");
                fm.push_str(&format!("  - \"{}\"\n", payload));
            ...
            if corrupt_date {
                fm.push_str(&format!("modified_datetime: \"{} garbage-date\"\n", payload));
            }
            fm.push_str("---\n");
    ...
    println!("Foundry: Generated {} test files across 64 corruption profiles.", file_count);
    ...
    let service = SyncService::new(
        ...
        config.clone(),
    ).await.expect("Sync Service MUST NOT panic during initialization, even with garbage files");

    // Start-up sync already happened in ::new(), but we can trigger it again to check for idempotency/hangs
    service.full_sync().await.expect("Sync Service MUST NOT fail globally due to individual file corruption");
    ...
    assert!(pages.len() > 0, "System should have recovered at least some valid pages");
    
    for page_feat in pages {
        if let Feature::Page(p) = page_feat {
            // Identifier check: Ensure no traversal escape
            assert!(!p.identifier.contains("../"), "Identifier sanitization failed for: {}", p.identifier);
            assert!(!p.identifier.contains("..\\"), "Identifier sanitization failed for: {}", p.identifier);
    ...
    println!("Foundry: All {} pages processed without system failure.", file_count);
}
...
async fn test_sync_corrupt_media_handles_gracefully() {
    ...
    reader.add_file("/content/videos/not_a_video.mp4", "# This is actually markdown");
    ...
    let service = SyncService::new(
        ...
        Box::new(notifier.clone()),
        config.clone(),
    ).await.expect("Sync service MUST survive corrupt media initialization");

    service.full_sync().await.expect("Sync service MUST survive corrupt media batch");
    ...
    assert!(feat.is_some(), "Should still expose corrupt assets as features");
